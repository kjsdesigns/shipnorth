...
router.post('/register', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (error.message === 'CUSTOMER_EXISTS') {
      return ResponseHelper.conflict(res, 'An account with this email already exists. Please sign in instead.');
    }
...
router.post('/import', authorize('staff', 'admin'), asyncHandler(async (req, res) => {
  ...
  ResponseHelper.success(res, results, `Import completed: ${results.created} created, ${results.updated} updated`);
}));

// List customers (staff only)
router.get('/', authorize('staff', 'admin'), asyncHandler(async (req, res) => {
...
router.get('/:id/packages', asyncHandler(async (req: AuthRequest, res) => {
  // Check access permissions
  if (req.user?.role === 'customer' && req.user.customerId !== req.params.id) {
    return ResponseHelper.forbidden(res, 'Access denied');
  }
...
router.put('/:id', asyncHandler(async (req: AuthRequest, res) => {
  ...
  const validation = ValidationHelper.validateCustomerRegistration({ ...req.body, email: 'temp@example.com' });
...
router.get('/:id/payment-methods', asyncHandler(async (req: AuthRequest, res) => {
  if (req.user?.role === 'customer' && req.user.customerId !== req.params.id) {
    return ResponseHelper.forbidden(res, 'Access denied');
  }
